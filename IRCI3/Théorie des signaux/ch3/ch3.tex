...
\end{equation}
où $-\infty<n<\infty$. Le signal de sortie est donné en effectuant la convolution
\begin{equation}
\begin{array}{ll}
...
Si par hasard le signal d'entrée peut se mettre sous la forme d'une somme d'exponentielles 
complexes (fonction propre) multipliée par un scalaire
\begin{equation}
x(n) = \sum_p a_pz_p^n
\end{equation}
...
\end{equation}
où $\displaystyle \alpha = e^{j(k-m)\frac{2\pi}{N}n}$. Il s'agit de la série géométrique
\begin{equation}
...
Notons que toute fonction de carré sommable est forcément absolument sommable.
	...
	revu ici.
		...
		d\omega
		\end{equation}
		Ceci montre que les énergies dans le domaine temporel correspondent aux énergies dans le 
		domaine fréquentiel. 
...
\end{equation}
Il existe d'autres façons de construire un tel signal. Calculons les coefficients $X_d(k)$ de la 
...
&= \DS \dfrac{1}{T}\int_{-T/2}^{T/2} \delta(t) e^{-i2\pi nt/T}\ dt \\
&= \DS \dfrac{1}{T} e^{-i2\pi n0/T}\\
&= \DS \dfrac{1}{T}
\end{array}
\end{equation}
En effet, une période correspond à l'intervalle $[-T/2,T/2]$. Sur cette intervalle on retrouve une 
seule impulsion (en zéro), les autres sont en dehors d'où le $\delta(t)$. On a donc
\begin{equation}
...
Revenons à notre somme de train d'impulsions. Comme le signal n'est non-nul que lorsque 
$\omega = k\frac{2\pi}{N}$ :
\begin{equation}
\begin{array}{ll}
...
coefficients de la série de Fourier. Le signal périodique $x(n)$ possède une série de raies espacées de $2\pi/N$.\\
